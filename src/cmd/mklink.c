...
static BOOL CreateJunction(LPCTSTR LinkName, LPCTSTR TargetName)
{
	...
	BOOLEAN (WINAPI *RtlDosPathNameToNtPathName_U)(PCWSTR, PUNICODE_STRING, PCWSTR *, CURDIR *)	= (BOOLEAN (WINAPI *)(PCWSTR, PUNICODE_STRING, PCWSTR *, CURDIR *))GetProcAddress(hNTDLL, "RtlDosPathNameToNtPathName_U");
	VOID (WINAPI *RtlFreeUnicodeString)(PUNICODE_STRING) = (VOID (WINAPI *)(PUNICODE_STRING))GetProcAddress(hNTDLL, "RtlFreeUnicodeString");

  TCHAR TargetFullPath[MAX_PATH] = {0};
#ifdef UNICODE
#define TargetFullPathW TargetFullPath 
...
#ifndef UNICODE
	    !MultiByteToWideChar(CP_ACP, 0, TargetFullPath, -1, TargetFullPathW, MAX_PATH) ||
#endif
  ...
  hJunction = CreateFile(LinkName, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS| FILE_FLAG_OPEN_REPARSE_POINT, NULL);
    ...
    wcscpy((WCHAR*)((BYTE*)Data->MountPointReparseBuffer.PathBuffer
      + Data->MountPointReparseBuffer.PrintNameOffset), TargetFullPathW);
