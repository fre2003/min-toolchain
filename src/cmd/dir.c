...
GetVolumePathName(
  ...
  DWORD cchBufferLength)
{
  PGetVolumePathName pGetVolumePathName = NULL;
  HINSTANCE hInstance;
  
  hInstance = GetModuleHandle(_T("KERNEL32"));
  if (hInstance != NULL)
  {
    pGetVolumePathName = (PGetVolumePathName) GetProcAddress(hInstance,
#ifdef _UNICODE
      "GetVolumePathNameW");
#else
    "GetVolumePathNameA");
    ...
    if (pGetVolumePathName != NULL)
    {
      if (pGetVolumePathName(lpszFileName, lpszVolumePathName, cchBufferLength) == TRUE)
        return TRUE;
    }
  }
