...
{
/* Thisfunction is called by CommandDelete and
does the actual process of deleting the single
  file */
  if (CheckCtrlBreak(BREAK_INPUT))
    return 1;
  
    /* checkto see if it is read only and if this is done based on /A
  ...
  if ((dwFlags & DEL_ATTRIBUTES) || (dwFlags & DEL_FORCE))
  {
    ...
    {
    /* settingfile to normal, not saving old attrs first
...
static DWORD DeleteFiles(LPTSTR FileName, DWORD* dwFlags, DWORD dwAttrFlags)
{
  ...
  if (hFile != INVALID_HANDLE_VALUE)
  {
    do
    {
      ...
      /* ifit is going to be excluded by - no need to check attrs */
      if (*dwFlags & DEL_ATTRIBUTES && !bExclusion)
      {
        
        /* saveif file attr check if user doesnt care about that attr anyways */
      ...
      /* usercant ask it to be quiet and tell you what it did */
    ...
    error_sfile_not_found(szFullPath);
  return dwFiles;
}
