...
#define PRINTF0(Format) { \
	UINT Len = _sntprintf(Out, OutEnd - Out, Format); \
	if (Len > (UINT)(OutEnd - Out)) return NULL; \
	Out += Len; }

#define PRINTF1(Format, v1) { \
	UINT Len = _sntprintf(Out, OutEnd - Out, Format, v1); \
	if (Len > (UINT)(OutEnd - Out)) return NULL; \
	Out += Len; }

#define PRINTF2(Format, v1, v2) { \
	UINT Len = _sntprintf(Out, OutEnd - Out, Format, v1, v2); \
	if (Len > (UINT)(OutEnd - Out)) return NULL; \
	Out += Len; }

#define PRINTF3(Format, v1,v2,v3) { \
	UINT Len = _sntprintf(Out, OutEnd - Out, Format, v1,v2,v3); \
	if (Len > (UINT)(OutEnd - Out)) return NULL; \
	Out += Len; }
...
#define RECURSE(Subcommand) { \
	...
	case C_PIPE:
		Sub = Cmd->Subcommands;
		RECURSE(Sub)
		PRINTF1(_T(" %s "), OpString[Cmd->Type - C_OP_LOWEST])
		RECURSE(Sub->Next)
		break;
	case C_IF:
		STRING(_T("if"))
		...
		if (Cmd->If.LeftArg && SubstituteForVars(Cmd->If.LeftArg, Buf))
			PRINTF1(_T(" %s"), Buf)
		PRINTF1(_T(" %s"), IfOperatorString[Cmd->If.Operator]);
	...
	case C_FOR:
		...
		if (Cmd->For.Params)
			PRINTF1(_T(" %s"), Cmd->For.Params)
		PRINTF2(_T(" %%%c in (%s) do "), Cmd->For.Variable, Cmd->For.List)
		RECURSE(Cmd->Subcommands)
		break;
	}
	...
	for (Redir = Cmd->Redirections; Redir; Redir = Redir->Next)
	{
		...
		PRINTF3(_T(" %c%s%s"), _T('0') + Redir->Number,
			RedirString[Redir->Type], Buf)
	}
	return Out;
}
