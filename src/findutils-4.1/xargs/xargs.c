...
static myboolean initial_args = true;
...
static myboolean print_args P_ ((myboolean ask));
static void do_exec P_ ((void));
static void add_proc P_ ((pid_t pid));
static void wait_for_proc P_ ((myboolean all));
...
read_line ()
{
	...
	int c = EOF;
	myboolean first = true;		/* true if reading first arg on line.  */
	int len;
	char *p = linebuf;
...
//child=_spawnvp (_P_WAIT, cmd_argv[0], cmd_argv);
		while ((child=Downhill_Process_Forkexec(cmd_argv[0], cmd_argv, NULL,0)) < 0  && procs_executing)
			wait_for_proc (false);
